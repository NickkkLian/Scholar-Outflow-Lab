...
桌面这类文件夹受 macOS TCC 保护，而 **TCC 是按可执行文件逐个授权的**。
LaunchAgent 直接跑 `/bin/bash` 会被拦，读不到 Desktop 下的脚本，**退出码 126**。
实测：com.scholaroutflow.daily 第一次注册后就是这么失败的（runs=1, exit 126），
而同机另一个任务用某个项目 venv 里的 python 跑了 13 次、退出码 0。

所以 plist 指向那个**已被授权的 python**，由它 spawn bash——
子进程按 responsible process 规则继承授权。这样不用用户再授权一次。

## 脆弱点（写在这里，免得日后莫名其妙失效）

依赖 LaunchAgent plist 里写的那个项目 venv 的 python 的 TCC 授权。那个 venv 若被重建或删除，
...
def main():
    ...
    try:
        ...
        log(f"⛔ TCC 拦截：读不了 {ROOT}。"
            "本任务依赖 LaunchAgent plist 里写的那个 python 的 TCC 授权，"
